...
function generateSEOChecks(data) {
    const checks = [];
    // Basic SEO checks
    if (data.title) {
        if (data.title.length >= 30 && data.title.length <= 60) {
            ...
            checks.push({
                category: 'Basic SEO',
                check: 'SEO Title Length',
                status: data.title.length === 0 ? 'critical' : 'recommended',
    ...
    if (data.description) {
        if (data.description.length >= 120 && data.description.length <= 160) {
            ...
            checks.push({
                category: 'Basic SEO',
                check: 'Meta Description Length',
                status: data.description.length === 0 ? 'critical' : 'recommended',
    ...
    const titleKeywords = data.keywords.slice(0, 5).some((kw) => data.title.toLowerCase().includes(kw.word) || data.description.toLowerCase().includes(kw.word));
    checks.push({
        category: 'Basic SEO',
        check: 'Keywords in Title/Description',
        status: titleKeywords ? 'good' : 'recommended',
    ...
    checks.push({
        category: 'Performance',
        check: 'HTTP Requests',
        status: data.estimatedRequests <= 20 ? 'good' : 'recommended',
